...
# define variables
args = {u'remoteHostImage': u'', u'target': u'/datasets/churn_predictlarge_out.csv', u'remote_input_data': [{u'datasource': u'datasource', u'dataset': u'CUST_CHURN_1M'}], u'output_datasource_type': u'', u'livyVersion': u'livyspark2', u'execution_type': u'DSX', u'source': u'', u'output_type': u'Localfile', u'remoteHost': u'', u'sysparm': u''}
...
outputDF = outputDF.drop("features")
outputDF = outputDF.drop("rawPrediction")
outputDF = outputDF.drop("probability")
outputDF.printSchema()

# save scoring result to given target
# scoring_df = outputDF.toPandas()
outputDF.write.format('csv').option('header',True).mode('overwrite').option('sep',',').save('file:////user-home/999/DSX_Projects/CustomerChurnLab/datasets/churn_predictlarge_out.csv')
